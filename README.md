<div align="center">

<img height="220" src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExanp1aXF0bWRrMDcxNmN6bG40dDZreWI3dWgxaWYwbnY5ejh1MDV3OCZlcD12MV9naWZzX3NlYXJjaCZjdD1n/78XCFBGOlS6keY1Bil/giphy.gif" />

# Gustavo Tavares

### 🚀 Desenvolvedor Full Stack em Formação

Desenvolvimento Web • Back-end • QA • IA • Sistemas SaaS

</div>

---

## 👨‍💻 Sobre Mim

Olá! Sou Gustavo Tavares, estudante do Ensino Médio e Desenvolvedor Full Stack em formação.

Atualmente atuo com desenvolvimento web, QA, manutenção de sistemas corporativos e soluções utilizando Inteligência Artificial.

Tenho experiência prática participando da evolução de sistemas ERP, CRM, Agendamentos, Ordens de Serviço, Controle Financeiro, Estoque, Landing Pages, Chatbots e aplicações voltadas para negócios.

Meu principal objetivo é me tornar um Desenvolvedor Full Stack especializado em sistemas web escaláveis e soluções SaaS.

---

## 🚀 Atualmente Trabalho Com

* HTML5
* CSS3
* JavaScript (ES6+)
* TypeScript
* Node.js
* Express.js
* Fastify
* React
* MySQL
* Prisma ORM
* Git & GitHub
* APIs REST
* Postman
* Cypress
* QA/Testes Funcionais
* Inteligência Artificial aplicada ao desenvolvimento

---

## 🛠 Tecnologias e Ferramentas

<div align="left">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="45" />
<img width="12" />

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="45" />
<img width="12" />

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="45" />
<img width="12" />

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" height="45" />
<img width="12" />

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="45" />
<img width="12" />

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" height="45" />
<img width="12" />

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" height="45" />
<img width="12" />

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="45" />
<img width="12" />

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" height="45" />
<img width="12" />

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" height="45" />
<img width="12" />

<img src="https://www.vectorlogo.zone/logos/getpostman/getpostman-icon.svg" height="45" />
<img width="12" />

<img src="https://cdn.simpleicons.org/prisma/2D3748" height="45" />

</div>

---

## 💼 Experiência

### CodXis Sistemas

* Desenvolvimento e manutenção de sistemas web
* Correção de bugs e melhorias
* QA e testes funcionais
* Integração de funcionalidades utilizando IA
* Evolução de sistemas ERP, CRM e Agendamentos
* Criação de landing pages responsivas
* Implementação de melhorias focadas em UX

---

## 📂 Projetos em Destaque

### 🏢 ERP SaaS

Sistema de gestão empresarial desenvolvido com:

* Node.js
* Fastify
* TypeScript
* Prisma
* MySQL
* JWT

Módulos:

* Produtos
* Estoque
* Clientes
* Fornecedores
* Financeiro
* Vendas

---

### 🎮 ArenaStorm Bot

Chatbot para arena gamer com:

* Node.js
* Express
* MySQL
* Painel Administrativo
* Sistema de Reservas
* API REST

---

### ⚽ Figurinhas Copa 2026

E-commerce Full Stack com:

* Front-end Web
* Node.js
* MySQL
* Painel Administrativo
* Gestão de Pedidos

---

### 👓 Ótica VIP

Landing Page moderna utilizando:

* React
* Vite
* Tailwind CSS
* Framer Motion

---

## 📚 Atualmente Estudando

* Arquitetura de Software
* Docker
* Desenvolvimento Back-end Avançado
* APIs REST
* Banco de Dados
* Sistemas SaaS
* Inteligência Artificial

---

## 🎯 Objetivo

Busco oportunidades como:

* Desenvolvedor Web Júnior
* Estagiário Full Stack
* Desenvolvedor Back-end Júnior
* QA Automation
* Desenvolvedor focado em IA

---

## 📫 Contato

📧 [gustavotmoura123@gmail.com](mailto:gustavotmoura123@gmail.com)

💼 LinkedIn: linkedin.com/in/gustavo-tavares497821

🐙 GitHub: github.com/GustavoTavares256

</div>
